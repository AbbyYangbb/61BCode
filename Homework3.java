...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int temp = ints[0];
    int num_unrp = 1; // sum_repl = len_ints - sum_unrp
    int len_ints = ints.length;
    for(int i = 1; i < len_ints; i++){ // i only used to parse, and
        if (temp != ints[i]){
            ints[num_unrp] = ints[i];
            temp = ints[i];
            num_unrp++;
        }
    }
    for(int i = num_unrp; i < len_ints; i++){
        ints[i] = -1;
    }
  }
